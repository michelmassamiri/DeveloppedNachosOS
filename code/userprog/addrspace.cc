...
AddrSpace::AddrSpace (OpenFile * executable)
{
  ...
  if(numPages > pageprovider->NumAvailPages())
    throw std::bad_alloc();
...
AddrSpace::InitRegisters ()
{
  ...
  #ifdef CHANGED
  //store thread main in bitmap[0]
  bitmap->Mark(currentThread->GetWhichThread());
  #endif //CHANGED
}
...
int
AddrSpace::AllocateUserStack(int which)
{
  bitmap->Mark(which) ;
  return (numPages*PageSize) -16 +(which * -256) ;
}

int AddrSpace::GetThreadCompteur(){
  return threadCompteur;
}

void AddrSpace::SetThreadCompteur(int cmp){
  threadCompteur = cmp;
}
